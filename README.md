...
站点页眉使用 `assets/logo.png` 品牌图片；该文件由提供的原图裁掉外围留白后生成。
...
页面支持一级、二级、三级类目联动筛选，以及按价格升序、降序排列。商品详情中的“加入选品池”会按 `row` 去重，并将选择保存在浏览器的 `localStorage`；选品池仅保存在当前浏览器和当前设备，不会上传到 GitHub 或在不同设备间同步。选品池卡片将 `row` 显示为 `SPU Code`，支持一键复制池内所有 SPU Code（每行一个），再次打开详情可将商品移出。
